...
@admin.action(description="Invalidate scrape jobs cache")
def invalidate_scrape_jobs_cache(modeladmin, request, queryset) -> None:
    invalidate_scrape_jobs()
    messages.success(request, "Scrape jobs cache invalidated.")


@admin.action(description="Trigger Insecam scrape now")
def trigger_insecam_scrape(modeladmin, request, queryset) -> None:
    ...
    messages.success(request, f"Insecam scrape task queued: {task.id}")


@admin.action(description="Trigger WhatsUpCams scrape now")
def trigger_whatsupcams_scrape(modeladmin, request, queryset) -> None:
    ...
    messages.success(request, f"WhatsUpCams scrape task queued: {task.id}")
